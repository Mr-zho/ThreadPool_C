...
void * manager_func(void *arg)
{
    ...
    while(1)
    {
        ...
        int busy_thread = t_pool->busy_thread;
        ...
        /* 增线程 */
        if(busy_thread * 2 < task_num  && current_thread_num < max_thread_num)
        {
        ...
        /* 减线程 */
        if(busy_thread > task_num * 2 && current_thread_num > min_thread_num)
        {
            int dl_thread_num = 
        }
...
int threadPool_Init(thread_pool **pool, int threadNums,int min_thread_num,int max_thread_num)
{
    ...
    pthread_create(&(t_pool->thread_manager),NULL,manager_func,t_pool);
...
{   
    ...
    while(freeNode != NULL)
    {
        ...
        free(freeNode);
        freeNode = pool->queue->next;
    }
    free(pool->queue);
#elif 0
    /* shuo */
    t_task *travel = pool->queue;
    while (pool->queue->next != NULL)
    {   
        t_task *freeNode = travel->next;
        travel = travel->next;
        free(freeNode);
    }
    free(pool->queue);
